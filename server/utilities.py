...
def db_connection():
    ...
    finished with them.
    """
    con = connection.MySQLConnection(user='fp_user', password='flickpick123', host='ec2-18-222-97-98.us-east-2.compute.amazonaws.com', database='FlickPick')
...
def process_single_tag(tag: str):
    ...
    :return: A dictionary with key, value pairs for 'tag_id', 'rating', and 'name'
    """
    split = tag.split(',')
    return {
            'id': split[0],
...
def process_movie_tags(movie: str):
    ...
    process_single_tag.
    :param str movie: This is a string of all the tags and their info for a movie.
    :return: A list of dictionaries with key, value pairs for 'tag_id', 'rating', and 'name'.
    """
    if movie is None:
        tag_data = []
    else:
